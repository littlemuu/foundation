...
编译器根据 `++` 出现在对象的前面还是后面，选择对应的函数：

```cpp
++it; // 前置：调用 it.operator++()
it++; // 后置：调用 it.operator++(0)
```

调用 `it++` 时，代码中虽然没有手动传入参数，但编译器会概念性地传入一个整数 `0`，因此匹配带 `int` 参数的版本：

```cpp
self_type operator++(int);
```

这个 `int` 参数只负责区分函数签名，不参与递增操作，所以通常只写类型，不写参数名。

编译器不能通过返回类型区分两个重载函数。下面的写法是错误的：

```cpp
self_type& operator++();
self_type operator++(); // 错误：参数列表相同，只有返回类型不同
```
...
1. 编译器看到 `++` 位于 `it` 后面，选择 `operator++(int)`。
2. 调用过程可以理解为 `it.operator++(0)`。
3. `self_type temp = *this` 保存递增前的迭代器副本。
4. `++(*this)` 调用前置 `operator++()`，把当前迭代器移动到下一个节点。
5. `return temp` 返回移动前的迭代器副本。
